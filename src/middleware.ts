...
export async function middleware(request: NextRequest, response: NextResponse) {
    ...
    if (!accessToken) {
        try {
            ...
            //     if (error.message === 'invalid token') {            //         console.log('не валидный токен')            //         request.cookies.delete(EnumTokens.ACCESS_TOKEN)            //         return redirectToLogin(isAdminPage, request)            //     }            // }            request.cookies.delete(EnumTokens.ACCESS_TOKEN)
            return redirectToLogin(isAdminPage, request)
        }    }
